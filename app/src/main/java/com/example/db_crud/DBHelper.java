...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void  addStudent(com.example.db_crud.StudentModel STUDENTModel){
    ...
    public ArrayList<com.example.db_crud.StudentModel> getAllStudents() {

        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursorCourses = db.rawQuery("SELECT * FROM " + STUDENT_TABLE, null);

        ArrayList<com.example.db_crud.StudentModel> studentArrayList = new ArrayList<>();

        // moving our cursor to first position.
        if (cursorCourses.moveToFirst()) {
            do {

                studentArrayList.add(new com.example.db_crud.StudentModel(cursorCourses.getInt(0), cursorCourses.getString(1),
